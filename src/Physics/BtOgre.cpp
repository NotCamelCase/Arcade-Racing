...
namespace BtOgre {
	...
	*/
	...
	btConvexHullShape* VertexIndexToShape::createConvex()
	{
		...
		btConvexShape* orgShape = new btConvexTriangleMeshShape(trimesh);
		btShapeHull* hull = new btShapeHull(orgShape);
		btScalar margin = orgShape->getMargin();
		hull->buildHull(margin);
		btConvexHullShape* shape = new btConvexHullShape((btScalar*)(hull->getVertexPointer()), hull->numVertices());
		shape->setLocalScaling(Convert::toBullet(mScale));
